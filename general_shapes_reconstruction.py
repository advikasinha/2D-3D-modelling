...
class DimensionExtractor:
    ...
    def create_operations_structure(self):
        """
        Create a structure with operations like extrude, cylinder, etc.
        ...
        # Add main extrude operation with the basic dimensions
        if any(v is not None for v in [self.matched_dimensions.get('width'), 
                                     self.matched_dimensions.get('height'), 
                                     self.matched_dimensions.get('depth')]):
            extrude_op = {
                "type": "extrude",
                "dimensions": {
                    "width": self.matched_dimensions.get('width', 0),
                    "height": self.matched_dimensions.get('height', 0),
                    "depth": self.matched_dimensions.get('depth', 0)
                },
                "position": {"x": 0, "y": 0, "z": 0}
            }
            structure["root"]["operations"].append(extrude_op)
        
        # Add cylinder operations for any circular features detected
        # (This is a simplified approach - in reality you'd need to detect circles)
        for view_type, view_data in self.dimensions.items():
            if 'extra_1' in view_data.get('extracted', {}):
                # Assume extra dimensions might be for circular features
                cylinder_op = {
                    "type": "cylinder",
                    "radius": view_data['extracted']['extra_1'] / 2,
                    "height": self.matched_dimensions.get('height', 20),
                    "position": {
                        "x": self.matched_dimensions.get('width', 0) / 2,
                        "y": self.matched_dimensions.get('depth', 0) / 2,
                        "z": 0
                    },
                    "axis": "Z"
                }
                structure["root"]["operations"].append(cylinder_op)
        
        # Add a simple sketch operation (simplified example)
        if 'width' in self.matched_dimensions and 'height' in self.matched_dimensions:
            sketch_op = {
                "type": "sketch",
                "contour": [
                    {
                        "type": "line", 
                        "start": {"x": 0, "y": 0}, 
                        "end": {"x": self.matched_dimensions['width'], "y": 0}
                    },
                    {
                        "type": "line", 
                        "start": {"x": self.matched_dimensions['width'], "y": 0}, 
                        "end": {"x": self.matched_dimensions['width'], "y": self.matched_dimensions['height']}
                    },
                    {
                        "type": "line", 
                        "start": {"x": self.matched_dimensions['width'], "y": self.matched_dimensions['height']}, 
                        "end": {"x": 0, "y": self.matched_dimensions['height']}
                    },
                    {
                        "type": "line", 
                        "start": {"x": 0, "y": self.matched_dimensions['height']}, 
                        "end": {"x": 0, "y": 0}
                    }
                ],
                "extrude": {
                    "depth": self.matched_dimensions.get('depth', 10)
                }
            }
            structure["root"]["operations"].append(sketch_op)
